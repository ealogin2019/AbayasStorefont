...
interface CartContextValue {
  items: CartItem[];
  add: (p: Product, opts?: { size?: string; color?: string; qty?: number; variantId?: string }) => void;
...
export function CartProvider({ children }: { children: React.ReactNode }) {
    ...
    const loadCart = async () => {
      ...
      try {
        ...
        const formattedItems: CartItem[] = (response.items || []).map((item: CartItemData & { product?: any }) => ({
          ...
          price: (item as any).variant?.price ? Number((item as any).variant.price) : item.product?.price || 0,
  ...
  const add = async (p: Product, opts?: { size?: string; color?: string; qty?: number; variantId?: string }) => {
    try {
      // Optimistically update UI
      setItems((prev) => {
        const itemId = `${p.id}::${opts?.variantId ?? ""}`;
